...
class FlowbiteComponent:
    """Base class for all Flowbite components"""
    
    @staticmethod
    def add_classes(*classes):
        """Combine CSS classes, filtering out None values"""
        return " ".join([c for c in classes if c])
    
    @staticmethod
    def merge_attributes(base_attrs, override_attrs=None):
        """Merge two dictionaries of HTML attributes, with override_attrs taking precedence"""
        if override_attrs is None:
            return base_attrs
        return {**base_attrs, **override_attrs}

# Common attribute sets
BUTTON_BASE_ATTRS = {
    "type": "button",
    "class": "focus:outline-none focus:ring-4 font-medium rounded-lg text-sm px-5 py-2.5 text-center"
}
...
# Navbar styles
NAVBAR_STYLES = {
    "default": "bg-white border-gray-200 px-4 lg:px-6 py-2.5 dark:bg-gray-800"
} 